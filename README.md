...
| --------------------------- | ------------- | ------------- |
| `DOMAIN`                    | SAMDOM.LOCAL  | Content Cell  |
| `DOMAIN_USER`               | Administrator  | Content Cell  |
| `DOMAIN_PASS`               | youshouldsetapassword  | Content Cell  |
| `DOMAIN_NETBIOS`            | SAMDOM  | Content Cell  |
| `HOSTIP`                    | NONE  | Content Cell  |
| `HOSTNAME`                  | $(hostname)  | Content Cell  |
| `JOIN`                      | false  | Content Cell  |
| `JOIN_SITE`                 | Default-First-Site-Name  | Content Cell  |
| `JOIN_SITE_VPN`             | false  | Content Cell  |
| `NTPSERVERLIST`             | 0.pool.ntp.org 1.pool.ntp.org 2.pool.ntp.org  | Content Cell  |
| `RECYCLEBIN`                | true  | Content Cell  |
| `DISABLE_MD5`               | true  | Content Cell  |
| `DISABLE_PW_COMPLEXITY`     | false  | Content Cell  |
| `ENABLE_CUPS`               | false  | Content Cell  |
| `ENABLE_DYNAMIC_PORTRANGE`  | NONE  | Content Cell  |
| `ENABLE_INSECURE_LDAP`      | false  | Content Cell  |
| `ENABLE_LAPS_SCHEMA`        | true  | Content Cell  |
| `ENABLE_LOGS`               | false  | Content Cell  |
| `ENABLE_MSCHAPV2`           | false  | Content Cell  |
| `ENABLE_RFC2307`            | true  | Content Cell  |
| `ENABLE_TLS`                | false | Content Cell  |
| `ENABLE_DEBUG`              | false  | Content Cell  |
| `DEBUG_LEVEL`               | 0  | Content Cell  |
| `ENABLE_BIND_INTERFACE`     | false  | Content Cell  |
| `BIND_INTERFACES`           | eth0  | Content Cell  |


* `DOMAIN` defaults to `CORP.EXAMPLE.COM` and should be set to your domain
* `DOMAIN_USER` Domain administrator username. Use docker secrets
* `DOMAIN_PASS` should be set to your administrator password, be it existing or new. This can be removed from the environment after the first setup run.
...
* `JOIN_SITE` is optional and can be set to a site name when joining and creating a domain, otherwise the default site name will be used.
* `JOIN_SITE_VPN` defaults to `false` and tells the container to connect to an OpenVPN site via an ovpn file with no password. For instance, if you have two locations where you run your domain controllers, they need to be able to interact. The VPN allows them to do that.

* `NTPSERVERLIST` Space seperated list of NTP servers.
* `RECYCLEBIN` AD RecycleBin optional Feature

* `DISABLE_MD5` Disables MD5 compability for higher security
* `DISABLE_PW_COMPLEXITY` defaults to `false`. When set to `true` it removes password complexity requirements including `complexity, history-length, min-pwd-age, max-pwd-age`

ENABLE_CUPS
* `ENABLE_DNSFORWARDER` is optional and if an IP such as `192.168.0.1` is supplied will forward all DNS requests samba can't resolve to that DNS server. Only add once. Add multiple forwarder speerated by spaces. Otherwise smb.conf will break
ENABLE_DYNAMIC_PORTRANGE
* `ENABLE_INSECURE_LDAP` defaults to `false`. When set to true, it removes the secure LDAP requirement. While this is not recommended for production it is required for some LDAP tools. You can remove it later from the smb.conf file stored in the config directory.
ENABLE_LAPS_SCHEMA
* `ENABLE_LOGS` defaults to `false`. When set to `true` it creates log file in /var/log/samba for kerberos and samba. Add a mapping
* `ENABLE_MSCHAPV2` defaults to `false`. When false nothing changes. When true ntlm auth = mschapv2-and-ntlmv2-only is added to config.
* `ENABLE_RFC2307` Enable RFC2307 support

* `ENABLE_TLS` defaults to `false`. When set to `true` it creates certificate files for ldaps

* `ENABLE_DEBUG
* `DEBUG_LEVEL

* `ENABLE_BIND_INTERFACE` boolean to bind to interfaces. Avoids messing up samba dns entries with random interface ip. defaults to false
* `BIND_INTERFACES` container internal interfaces to bind to. docker interface names are given alphabetically. e.g. docker-network-name=a => eth0 in container; docker-network-name=b => eth1 in container . Valid values would be `eth0` or a list seperated by space `eth0 eth1`
